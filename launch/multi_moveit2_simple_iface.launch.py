...
#
# Launches one moveit2_simple_iface (+ optional joy) per ROS namespace, e.g. abb1;abb2
...
#
# Same config convention as UR: robot_name:=abb loads config/abb/abb_sim.yaml for every namespace.
...
def launch_setup(context: LaunchContext, *args, **kwargs):
    ...
    if not parts:
        raise ValueError(
            "robot_namespaces must contain at least one namespace, e.g. abb1;abb2"
        )
    ...
    for idx, arg_robot_ns in enumerate(parts):
        ...
        # Match moveit2_simple_iface.launch.py: namespaced TF uses e.g. abb1_tool0, not bare tool0.
        with open(config_path, "r", encoding="utf-8") as base_cfg_file:
            base_cfg = yaml.safe_load(base_cfg_file) or {}
        base_cfg.setdefault("robot", {})
        base_cfg["robot"]["ee_link_name"] = f"{arg_robot_ns}_tool0"
        fd, tmp_cfg_path = tempfile.mkstemp(
            suffix=f"_{arg_robot_ns}_moveit2_simple_iface.yaml",
        ...
        servo_params = copy.deepcopy(servo_params_base)
        servo_params["moveit_servo"]["joint_topic"] = f"/{arg_robot_ns}/joint_states"
        ...
        remappings = [("joint_states", f"/{arg_robot_ns}/joint_states")]
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "robot_namespaces",
                default_value="abb1;abb2",
                description="Semicolon-separated ROS namespaces (e.g. abb1;abb2;abb3).",
            ),
            DeclareLaunchArgument(
                "robot_name",
                default_value="abb",
                description="arm_api2 config folder key (e.g. ur, abb) — same as moveit2_simple_iface.launch.py.",
            ),
            DeclareLaunchArgument(
                "launch_joy",
                default_value="false",
